...
class LiepinSpider(scrapy.Spider):
    ...
    if IS_ONLY_TODAY:
        ...
        start_urls.append('https://www.liepin.com/zhaopin/?ckid=3f43d0b1382876af&fromSearchBtn=2&degradeFlag=0&init=-1&key={}&headckid=3f43d0b1382876af&curPage=0'.format(KEYWORD))
...
# https://www.liepin.com/zhaopin/?key={}
    def parse(self, response):
        ...
        if soup_next:
            ...
            if next_p.startswith('/zhaopin/'):
                next_o = "https://www.liepin.com" + next_p
                yield Request(next_o, callback=self.parse, dont_filter=True,
    ...
    def parse_detail(self, responce):
        ...
        # 职位诱惑
        soup_y = soup.find('div', class_="comp-tag-list clearfix")### 需要改
        if soup_y:
            offer['temptation'] = soup_y.get_text(';', strip=True)

        # 职位职责
        soup_div_job = soup.find('div', class_="job-item main-message job-description")  or soup.find('div', class_='job-main main-message')
        if soup_div_job:
        ...
        soup_firm = soup.find('div', class_='new-compwrap')
        # 公司名
        soup_firm_name = soup_firm.find('div', class_='company-logo')
        firm['firm_name'] = soup_firm_name.get_text(strip=True).replace('关注', '')
        indus, scale, nature = '', '', ''
        # 公司行业, 规模， 性质
        if not soup_firm:
            ...
            soup_ul = soup_firm.find('ul', class_='new-compintro')
